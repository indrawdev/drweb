...
Ext.onReady(function() {
	...
	var grupPesan = Ext.create('Ext.data.Store', {
		autoLoad: false,
		fields: [
			'fs_kd_pesan','fd_tgl_periksa','fd_tgl_pesan',
		...
		proxy: {
			...
			type: 'ajax',
			url: 'trspesan/KodePesan'
		},
		listeners: {
			beforeload: function(store, operation) {
				Ext.apply(store.getProxy().extraParams, {
					'fs_kd_pesan': Ext.getCmp('cboPesan').getValue(),
					'fs_nama': Ext.getCmp('cboPesan').getValue(),
	...
	var winGrid = Ext.create('Ext.ux.LiveSearchGridPanel', {
		...
		columns: [
			...
			{text: 'Tgl Periksa', dataIndex: 'fd_tgl_periksa', menuDisabled: true, width: 80},
			{text: 'Tgl Daftar', dataIndex: 'fd_tgl_pesan', menuDisabled: true, width: 80},
		...
		listeners: {
			itemdblclick: function(grid, record)
			{
				...
				Ext.getCmp('txtTgl2').setValue(record.get('fd_tgl_pesan'));
	...
	function fnSimpan() {
		...
		Ext.Ajax.request({
			...
			params: {
				'fs_kd_pesan': Ext.getCmp('cboPesan').getValue(),
				'fd_tgl_periksa': Ext.Date.format(Ext.getCmp('txtTgl').getValue(), 'Y-m-d'),
				'fd_tgl_pesan': Ext.Date.format(Ext.getCmp('txtTgl2').getValue(), 'Y-m-d'),
				'fs_kd_mr': Ext.getCmp('cboMR').getValue(),
				'fs_nm_pasien': Ext.getCmp('txtNama').getValue(),
				'fs_alamat': Ext.getCmp('txtAlamat').getValue(),
			...
			success: function(response, opts) {
				...
				if (xText.sukses === true) {
					...
					fnReset();
					Ext.getCmp('txtPesan').setValue(trim(xText.kodepesan));
				}
			},
			failure: function(response, opts) {
				var xText = Ext.decode(response.responseText);
	...
	var frmTrsPesan = Ext.create('Ext.form.Panel', {
		border: false,
		frame: true,
		region: 'center',
	...
	Ext.TaskManager.start({
		run: function() {
			Ext.getCmp('txtTgl2').setValue(Ext.Date.format(new Date(), 'd-m-Y'));
		},
		interval: 1000
	});
	...
	frmTrsPesan.render(Ext.getBody());
